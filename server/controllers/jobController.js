...
export const jobController = {
    ...
    addJob: async (req, res) => {
        ...
        //if (!user_role) return res.status(400).json({ message: "user role is not defined" });
        ...
        try {
            ...
            // const isExistJob = await Job.findOne({ title: title } && { description: description });
            // if (isExistJob) return res.status(400).json({ message: "job already exist" });
            //if (isExistJob) return sendResponse(res, 400, false, "job already exists");
    ...
    getPaginatedJobs: async (req, res) => {
    ...
    serachJobs: async (req, res) => {
        ...
        try {
            // const allJobs = await Job.find({
            //     $or: [{ title: searchValue}, { category: searchValue }, { location: searchValue }, { level: searchValue }]
            // });
    ...
    searchJobsByTitleAndLocation: async (req, res) => {
    ...
    filterJobs: async (req, res) => {
        ...
        // if (!filterdJobCategory) return res.status(400).json({ message: "No provided anything to filter" });
        ...
        try {
            ...
            // const filteredResult = allJobs.filter((job) => (job.category.toString().toLowerCase() === filterdJobCategory.toString().toLowerCase()) || (job.location.toString().toLowerCase() === filterdJobCategory.toString().toLowerCase()));
            // if (filteredResult.length === 0) return res.status(404).json({ message: "No results found" });
    ...
    jobs: async (req, res) => {
        ...
        // if(!page) return sendResponse(res, 400, false, "page not found");
        // if(!limit) return sendResponse(res, 400, false, "limit not found");
        // if(!search) return sendResponse(res, 400, false, "search not found");
        // if(!title) return sendResponse(res, 400, false, "title not found");
        // if(!category) return sendResponse(res, 400, false, "category not found");
        // if(!location) return sendResponse(res, 400, false, "location not found");


        //fetching all jobs
        // if (!page && !limit && !search && !title && !category && !location) {
        //     const result = await Job.find({})

        //     if (!result) return sendResponse(res, 404, false, "Jobs not found");

        //     return sendResponse(res, 200, true, "All jobs", { allJobs: result });
        // }
